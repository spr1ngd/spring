...
void PostProcessing::PreProcess() 
{
	...
	if (this->antiAliasing->enabled) this->srcFramebuffer = FrameBufferObject::GenMSColorFramebuffer(this->antiAliasing->samples, Screen::width, Screen::height);
	else this->srcFramebuffer = FrameBufferObject::GenColorFramebuffer(Screen::width, Screen::height, 0);
